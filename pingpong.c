...
int main(int argc, char const *argv[]) {
  ...
  char Buffer_padre[10], Buffer_hijo[10];
  ...
  if (hijo == 0) {
    ...
    while (1) {
      ...
      strcpy(Pong, "PONG\n");
      escribir = write(tuberia_hijo[1], Buffer_hijo, strlen(Buffer_hijo));
      if (escribir == -1) {
        perror("Error al escribir en el hijo.");
        exit(EXIT_FAILURE);
      }
    ...
    while (1) {
      ...
      strcpy(Buffer_padre, "PING\n");
      escribir = write(tuberia_padre[1], Buffer_padre, strlen(Buffer_padre));
      if (escribir == -1) {
